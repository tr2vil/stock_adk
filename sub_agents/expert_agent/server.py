...
_logger = get_logger("expert_agent.server")
...
async def reload_prompt(request: Request) -> JSONResponse:
    ...
    new_prompt = await aget_prompt("expert_agent")
    if new_prompt:
        root_agent.instruction = new_prompt
        _logger.info("prompt_reloaded", agent="expert_agent", length=len(new_prompt))
        return JSONResponse({"status": "reloaded", "agent": "expert_agent"})
    return JSONResponse({"status": "no_change", "agent": "expert_agent"})

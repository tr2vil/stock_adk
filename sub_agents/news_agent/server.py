...
_logger = get_logger("news_agent.server")
...
async def reload_prompt(request: Request) -> JSONResponse:
    ...
    new_prompt = await aget_prompt("news_agent")
    if new_prompt:
        root_agent.instruction = new_prompt
        _logger.info("prompt_reloaded", agent="news_agent", length=len(new_prompt))
        return JSONResponse({"status": "reloaded", "agent": "news_agent"})
    return JSONResponse({"status": "no_change", "agent": "news_agent"})

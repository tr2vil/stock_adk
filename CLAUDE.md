...
├── docs/                      # 설계 문서·스펙 (TRADING_SYSTEM_SPEC.md 등)
├── docker-compose.yml
├── pyproject.toml
└── .env.example
```
...
`tests/manual/test_agent.py` 스크립트로 간편하게 테스트할 수 있습니다:

```bash
# 기본 사용법
python tests/manual/test_agent.py <port> "<message>"

# 디버그 모드 (전체 JSON 응답 확인)
python tests/manual/test_agent.py <port> --debug "<message>"
```
...
python tests/manual/test_agent.py 8003 "NVDA 기술적 분석해줘"

# Expert Agent (8004)
python tests/manual/test_agent.py 8004 "Get analyst ratings for MSFT"
python tests/manual/test_agent.py 8004 "애플 애널리스트 의견 분석해줘"

# Risk Agent (8005)
python tests/manual/test_agent.py 8005 "Calculate position size for AAPL with 100000 capital"
python tests/manual/test_agent.py 8005 "10만달러 자본으로 TSLA 포지션 사이징해줘"
```
